...
-- Function: Get Active Loan Count return nbr total des pret nom returner
...
BEGIN
    v_days_overdue := TRUNC(p_return_date - p_due_date); -- Trunk :Elle supprime la partie décimale et garde seulement la partie entière.
...
BEGIN
    ...
    --v_membership_expiry devient la date d’expiration de l’abonnement du nouveau patron. exemple Si aujourd’hui : SYSDATE = 25‑NOV‑2025
                                                                                                      --v_membership_expiry = 25‑NOV‑2026
...
BEGIN
    ...
    SET email = NVL(p_email, email),
        ...
        -- NVL(val1, val2) retourne val1 si elle n’est pas NULL, sinon elle retourne val2
...
CREATE OR REPLACE PROCEDURE sp_renew_membership (
    ...
    v_account_status VARCHAR2(20); -- check account status first
    v_fines_owed NUMBER; -- check fines awed 
BEGIN
...
-- sp_suspend_patron sert à suspendre un compte de patron dans la base de données.
...
CREATE OR REPLACE PROCEDURE sp_renew_loan (
    ...
    v_fines_owed NUMBER;
    v_membership_type VARCHAR2(20); -- Déclarer la variable
...
BEGIN
    ...
    -- select le type de membership
        SELECT membership_type 
    INTO v_membership_type 
    FROM PATRONS 
    WHERE patron_id = v_patron_id;

    -- Déterminer la nouvelle date de retour selon le type d'abonnement
    IF v_membership_type = 'VIP' THEN
        p_new_due_date := SYSDATE + 15;
    ELSIF v_membership_type = 'Premium' THEN
        p_new_due_date := SYSDATE + 10;
    ELSE
        p_new_due_date := SYSDATE + 6;
    END IF;
    
    -- Mettre à jour le prêt
    UPDATE LOANS 
    SET due_date = p_new_due_date, renewal_count = renewal_count + 1
    WHERE loan_id = p_loan_id;
...
--Supprime un document et ses copies associées uniquement s’il n’a aucun prêt actif.
-- Supprime aussi les liens avec les auteurs et les genres.
...
BEGIN
    -- Vérifier que le patron existe et récupérer le total des amendes
    SELECT 
        NVL(total_fines_owed, 0)
    INTO v_total_fines
    FROM PATRONS
    WHERE patron_id = p_patron_id;

    -- Nombre d’emprunts actifs
    v_active_loans := fn_get_active_loan_count(p_patron_id);
